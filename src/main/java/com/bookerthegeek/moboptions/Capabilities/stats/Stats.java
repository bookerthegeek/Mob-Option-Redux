...
public class Stats implements IHealth, IAttack, IFollow{
	...
	protected static final String MODIFIER_FOLLOW_NAME = "Bonus Max Health";
	...
	private final IAttributeInstance dummyAttackAttribute = new AttributeMap()
			.registerAttribute(SharedMonsterAttributes.ATTACK_DAMAGE);
	
	private final IAttributeInstance dummyFollowAttribute = new AttributeMap()
	...
	public final void setBonusMaxHealth(final float bonusMaxHealth) {
		...
		onStatChanged(MODIFIER_HEALTH_ID, SharedMonsterAttributes.MAX_HEALTH, dummyMaxHealthAttribute, "health", MIN_HEALTH_AMOUNT);
	}
	...
	public void setBonusMaxAttack(final float bonusMaxAttack) {
		...
		onStatChanged(MODIFIER_ATTACK_ID, SharedMonsterAttributes.ATTACK_DAMAGE, dummyAttackAttribute, "attack", MIN_ATTACK_AMOUNT);
	}
	...
	public void setBonusFollowRange(final float bonusFollowRange) {
		...
		onStatChanged(MODIFIER_FOLLOW_ID, SharedMonsterAttributes.FOLLOW_RANGE, dummyFollowAttribute, "follow", MIN_FOLLOW_AMOUNT);
	}
	...
	public void synchronise() {
		if (entity != null && !entity.getEntityWorld().isRemote) {
			final IAttributeInstance health = entity
					.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH);
			
			final IAttributeInstance attack = entity
					.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE);
			
			final IAttributeInstance follow = entity
			...
			final SPacketEntityProperties packet = new SPacketEntityProperties(entity.getEntityId(),
					Arrays.asList(new IAttributeInstance[]{health, attack, follow}));
	...
	protected void onStatChanged(UUID id, IAttribute type, IAttributeInstance dummy, String mod, float minAmountForType) {
		if (entity == null)
			return;

		final IAttributeInstance entityAttribute = entity
		...
		entityAttribute.getModifiers().stream().filter(modifier -> !modifier.getID().equals(id))
				.forEach(dummy::applyModifier);
